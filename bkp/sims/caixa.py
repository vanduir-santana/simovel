...
class SimuladorResultadoItem():
    """Representa cada linha da tabela 2x2 com o resultado da simulação"""
...
class SimulacaoResultadoCaixa():
    """Organiza os dados html retornados da simulação da caixa, converte
    pros respectivos tipos e exibe o resultado.
    """
    ...
    def opcao_financiamento(self) -> OpcaoFinanciamento:
        return getattr(
            self, 
            '_opcao_financiamento',
             OpcaoFinanciamento.SBPE_CRED_IMOB_POUP_CAIXA
        )
    ...
    def __str__(self):
        ...
        s = (
            ...
            f'{self.titulo}\n'
            f'{"#" * len(self.titulo)}\n'
            f'{self.valor_imovel.descricao:>{tam}}: {self.valor_imovel.valor}\n'
            f'{self.prazo_maximo.descricao:>{tam}}: {self.prazo_maximo.valor}\n'
            f'{self.prazo_escolhido.descricao:>{tam}}: {self.prazo_escolhido.valor}\n'
            f'{self.cota_maxima.descricao:>{tam}}: {self.cota_maxima.valor}\n'
            f'{self.valor_entrada.descricao:>{tam}}: {self.valor_entrada.valor}\n'
        )
        if self.valor_subsidio.valor:
            s += f'{self.valor_subsidio.descricao:>{tam}}: {self.valor_subsidio.valor}\n'
        s += (
            f'{self.valor_financiamento.descricao:>{tam}}: {self.valor_financiamento.valor}\n'
            f'{self.sistema_amortizacao_indexador.descricao:>{tam}}: {self.sistema_amortizacao_indexador.valor}\n'
            '\n'
            f'{self.primeira_prestacao.descricao:>{tam}}: {self.primeira_prestacao.valor}\n'
            f'{self.ultima_prestacao.descricao:>{tam}}: {self.ultima_prestacao.valor}'
        )
        return s
...
class SimuladorCaixa(SimuladorBase):
    ...
    def simular(self) -> SimulacaoResultadoCaixa:
        ...
        va_conta_fgts = 'V' if self.tres_anos_fgts else ''
        grupo_tipo_financiamento = self.tipo_financiamento.value
